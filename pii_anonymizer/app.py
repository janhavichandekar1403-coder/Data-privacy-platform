...
def process():
    ...
    # Calculate residual privacy risk based on protection methods
    residual_risk = {
        "skip": 1.00,
        "mask": 0.25,
        "encrypt": 0.10,
    ...
    original_detected = detect_pii_columns(df)

    risk_values = []

    for item in original_detected:
        ...
        method = column_methods.get(column, "skip")
        factor = residual_risk.get(method, 1.0)

        risk_values.append(confidence * factor)

    if risk_values:
        risk_after = round(sum(risk_values) / len(risk_values), 1)
    else:
        risk_after = 0
